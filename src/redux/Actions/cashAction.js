...
export const createCashFlow = (order) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(`https://up-views.herokuapp.com/api/cashFlow`, order, config);
...
export const listCashFlow = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`https://up-views.herokuapp.com/api/cashFlow`, config);
...
export const getMoney = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`https://up-views.herokuapp.com/api/cashFlow/getByUserDeposit`, config);
